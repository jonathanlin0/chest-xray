...
class chexpert_dataset(Dataset):
    def __init__(self, paths, pathology_idx, convert_to_float=True, root_path="data/", transform=None):
        """
    ...
    def __len__(self):
        ...
        Required to implement for Dataset object.
    ...
    def __getitem__(self, idx):
        """
        Returns the item from the dataset corresponding to given index.
        Required to implement for Dataset object.
        ...
        Returns:
        2-tuple: first element is the tensor representation of the image, and the second element is the label
        """
...
# returns the training and validation dataloader. The training and validation datasets are subsets of the training data. This is different than the validation dataset for evaluating performance on Eval.ai
...
class ResNet50_Modified(nn.Module):
    def __init__(self):
        ...
        # Do not freeze any layers in the pretrained model
        # for param in self.resnet.parameters():
        #     param.requires_grad = False
        
        # Replace the final fully connected layer of ResNet-50
        # Original ResNet-50 has 2048 output features before the final FC layer
...
def train_model():
    ...
    for epoch in range(EPOCHS):  # run for 5 epochs
        ...
        if epoch % TEST_PERIOD == 0 and epoch != 0:
            ...
            test_csv_path = "2024/pp/solution_ids.csv"
            paths = []
            ...
            f = open(f"2024/pp/labels/output_labels_{MODE}{CATEGORY_IDX}.json", "r")
            old = json.load(f)
            f.close()
            ...
            # save the values
            with open(f"2024/pp/labels/output_labels_{MODE}{CATEGORY_IDX}.json", "w") as outfile:
